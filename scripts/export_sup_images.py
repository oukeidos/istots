...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export deduplicated subtitle images from SUP. "
            "Uses istots Python parser/policy path (same dedupe basis as conversion)."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Export only first N deduplicated frames",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def export_images(input_sup: Path, output_dir: Path, max_items: int | None) -> int:
    ...
    def on_total(count: int) -> None:
        nonlocal total_hint
        total_hint = count
        logging.info("deduplicated frames detected: %d", count)
